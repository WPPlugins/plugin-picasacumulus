			...
			'ok': function() {
				...
				while (count < 3) {
					...
					if (result[0] == true) {
						...
						$('#pluginpicasacumulus-dialog-login-error-message').text('Identification invalide, essai: ' + (count + 1) + ' / 3');
						$('#pluginpicasacumulus-dialog-login-error').show();
					}
					count++;
				}
				$('#pluginpicasacumulus-dialog-login-error-message').text('Erreur Identification, veuillez ré-actualiser la page!');
				$('#pluginpicasacumulus-dialog-login-error').show();
			},
			'cancel': function() {
				$(this).dialog('close');
